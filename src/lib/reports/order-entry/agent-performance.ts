...
async function run(params: ReportParams): Promise<ReportResult> {
  ...
  const rows: ReportRow[] = raw.map((r) => ({
    ...
    top_customer_share:
      n(r.value) > 0 ? Math.round((n(r.top_customer_value) / n(r.value)) * 1000) / 10 : null,
...
export const agentPerformance: ReportDefinition = {
  ...
  columns: [
    ...
    { key: "avg_order", label: "Avg order", type: "money" },
    { key: "avg_rate", label: "Avg rate", type: "money" },
    { key: "top_customer", label: "Biggest customer", type: "text", width: 30 },
    { key: "top_customer_share", label: "From that one", type: "percent", note: "How much of THIS person's value comes from their biggest customer." },
    ...
    { key: "days_since_last", label: "Days quiet", type: "int" },
  ],
  filters: [
