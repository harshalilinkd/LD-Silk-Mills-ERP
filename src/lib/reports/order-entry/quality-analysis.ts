...
async function run(params: ReportParams): Promise<ReportResult> {
  ...
  const rows: ReportRow[] = raw.slice(0, MAX_EXPORT_ROWS).map((r) => ({
    ...
    share: total > 0 ? Math.round((n(r.value) / total) * 1000) / 10 : null,
...
export const qualityAnalysis: ReportDefinition = {
  ...
  columns: [
    ...
    { key: "avg_rate", label: "Avg rate", type: "money", note: "Value divided by metres — weighted, so a big line counts more." },
    { key: "median_rate", label: "Middle rate", type: "money", note: "The median line's rate — unweighted, so one big line cannot drag it." },
    { key: "min_rate", label: "Lowest rate", type: "money" },
    { key: "max_rate", label: "Highest rate", type: "money" },
    { key: "rate_spread", label: "Spread", type: "money", note: "Highest minus lowest." },
