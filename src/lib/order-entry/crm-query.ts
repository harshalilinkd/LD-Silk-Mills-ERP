// Ported near-verbatim from Order Entry's lib/crm-query.ts. Only the imports
// changed (this repo's paths); the aggregation logic, the deliberate
// "batch queries to fit the 5-connection pool" discipline, and the SQL
// itself are unchanged — same live tables, same rules.
//
// ⚠️ Concurrency: src/db/order-entry/index.ts reuses the shared postgres.js
// pool, capped at max:5 for the WHOLE process, and through the Supavisor
// pooler the surplus does not queue — it stalls. Every function here runs
// its queries SEQUENTIALLY or in waves of ≤4. The reconcile is deliberately
// two statements, not a fan-out.
import {
  and,
  asc,
  count,
...
// ---------------------------------------------------------------------------
// One follow-up's detail (CLAUDE.md §12.5.2/§12.6)
// ---------------------------------------------------------------------------

export type FollowupDetail = {
  followup: typeof crmFollowups.$inferSelect;
  sla: {
    stageKey: string;
    label: string;
    targetDays: number;
  ...
  attempts: (typeof crmFollowupAttempts.$inferSelect)[];
  issues: (typeof crmIssues.$inferSelect)[];
...
/**
 * Everything the detail page needs in one server-side call: the follow-up,
 * the order it belongs to, that order's design lines (so an issue can be
 * pinned to a quality/design), the attempts logged so far, the issues already
 * raised, the scores given, and which stages actually missed their deadline.
 *
 * This is the SAME shape GET /api/crm/followups/:id returns — both call this
 * function rather than keeping two copies of the aggregation, one for the
 * page's initial render (this call, direct — no HTTP round trip) and one for
 * the route (kept for any client-side consumer that still wants a fetchable
 * GET). Returns null when the follow-up doesn't exist.
 */
export async function loadFollowupDetail(id: string): Promise<FollowupDetail | null> {
  const rows = await db
  ...
  // Sequential, well inside the ≤4 fan-out rule — the pool is the scarce
  // resource here, not the round trips.
  ...
  // The scores given so far, plus the criteria to render them against. Both
  // are needed: a score whose criterion has since been retired must still be
  // shown, and an unscored active criterion must still appear.
  ...
  // this the page could only say "our SLA says late" — true, unactionable,
  // and impossible for a coordinator to check. The numbers let them see that
  // dispatch ran 2 days over rather than guessing.
  const sla = await db
    .select({
