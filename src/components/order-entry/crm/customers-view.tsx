...
// complaints from the CRM. **Never a second customer master.**
//
...
//   2. **Rows group on `crr_customer_id` where we have one.** Where we do not,
...
function Trend({ v }: { v: number | null }) {
  ...
  // which would claim a stability we have no evidence for, and never blank.
  if (v === null) return <span className="text-text-2">—</span>;
  if (Math.abs(v) < 0.25) {
    return (
      <span className="inline-flex items-center gap-1 text-text-2">
        <IconMinus className="size-3.5" /> steady
      </span>
    );
  }
  const up = v > 0;
...
export function CustomersView() {
  ...
  // Clears all three filters FIRST and then applies the new one unless it was
  // already active — so a tile is never a trap you cannot get out of, and two
  // tiles can never be half-applied at once.
  const only =
    (set: (v: string) => void, value: string, current: string) => () => {
    ...
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-2 sm:gap-3 xl:grid-cols-4">
        ...
        <StatCard
          ...
          sub={
            rated === "any"
              ? "showing rated only"
              : "customers with a completed call"
          }
          tone="warning"
          active={rated === "any"}
      ...
      {k && k.rated === 0 ? (
        <div className="rounded-card border border-border bg-surface px-4 py-3 text-[12.5px] text-text-2 shadow-sm">
          ...
          <b className="text-text-1">Follow-ups</b> queue is worked.
        </div>
      ) : null}
