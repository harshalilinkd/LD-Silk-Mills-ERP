...
//
// A popover of checkboxes over a table's column list. Thirteen columns is more
// than most people want at once, and they are what push a table past the
// viewport — so switching a few off is the difference between a table that
// scrolls sideways and one that does not.
//
// ── IT LIVES IN `shared/` BECAUSE THREE TABLES USE IT ─────────────────────
//
// It was written for the Order status board and sat under `order-status/`,
// which read as "this belongs to that screen". The owner then asked for the
// same control on **All orders** and on **Operations** (Sep 2026), and all
// three tables are the same thirteen-ish columns over the same orders — a
// person who switches Haste off on one is telling you something about Haste,
// not about that screen.
//
// It takes nothing but props: the STATE lives in `useColumnPrefs` beside it,
// once per table under its own storage key, because the column SETS differ
// (the board has Stages and Overall; the other two have Agent and Actions)
// and one shared key would leave orphan ids in everybody's storage.
//
// `order-status/column-picker.tsx` re-exports this so that screen's import
// keeps working.
//
