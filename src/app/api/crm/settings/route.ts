...
import { crmSettings } from "@/db/order-entry/schema";
import { crmSettingsUpdateSchema, firstZodError } from "@/lib/order-entry/validation";
...
//
// crm_settings holds exactly one row. Both handlers read it by "first row"
// rather than by a known id, because the id is random and nothing else
// references it.
//
// No admin UI writes to this route yet — the CRM settings editor is deferred
// to the Settings hub build (see CLAUDE.md). The route exists now because the
// follow-up detail page reads config through loadCrmConfig(), which this
// table backs.
