...
import { plannedAtForOffset } from "@/lib/order-entry/workflow";
import {
  customerOrders,
  lineStageProgress,
  orderLineItems,
...
// POST /api/order-entry/stages/recompute — re-derive planned_at for all
// NOT-yet-done stages of existing orders from the current SLA config (ADMIN).
// Lets orders created before an SLA change pick up the new offsets. Completed
...
export async function POST() {
  ...
  try {
    ...
    console.error("POST /api/order-entry/stages/recompute failed:", e);
