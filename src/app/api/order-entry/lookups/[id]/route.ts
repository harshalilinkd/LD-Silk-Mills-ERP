...
import { firstZodError, lookupUpdateSchema } from "@/lib/order-entry/validation";
import { lookupValues } from "@/db/order-entry/schema";

type Params = { params: Promise<{ id: string }> };

// PATCH /api/order-entry/lookups/:id — edit value and/or active flag (ADMIN).
...
// DELETE /api/order-entry/lookups/:id — soft delete (is_active=false) by
// default, or a permanent delete with ?hard=1. ADMIN. Lookups aren't
// FK-referenced (orders store party/fabric as text), so a hard delete is safe.
