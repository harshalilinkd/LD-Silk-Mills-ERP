...
import { isUniqueViolation, jsonData, jsonError, requireRole } from "@/lib/order-entry/api";
import { orderEntryDb as db } from "@/db/order-entry";
import { ROLES } from "@/lib/order-entry/rbac";
import { firstZodError, lookupCreateSchema } from "@/lib/order-entry/validation";
import { LOOKUP_CATEGORIES, lookupValues } from "@/db/order-entry/schema";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("POST /api/order-entry/lookups failed:", e);
