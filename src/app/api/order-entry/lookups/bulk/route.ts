...
// POST /api/order-entry/lookups/bulk — paste many values (one per line) for a
// category. Idempotent: dedupes within the paste and against existing values,
// reactivates soft-deleted matches, and reports added vs skipped. ADMIN only.
...
// DELETE /api/order-entry/lookups/bulk — remove many values at once by id.
// Soft-deletes (is_active=false) by default, or permanently with
// { hard: true }. ADMIN only. Lookups aren't FK-referenced (orders store
// party/fabric as text), so a hard delete is safe.
