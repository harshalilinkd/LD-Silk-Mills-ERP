// Ported from Order Entry's app/api/orders/[id]/tracking/route.ts — only the
// import paths and the db handle changed.
...
// GET /api/order-entry/orders/:id/tracking — order header + each line with its
// 7 ordered stages and derived operations status (per line and rolled up).
  ...
  const lineOut = lines.map((l) => {
    ...
    return {
      ...
      stages: rows.map((s) => ({
        ...
        label:
          STAGE_LABELS[s.stageKey as keyof typeof STAGE_LABELS] ?? s.stageKey,
