...
import { firstZodError, userUpdateSchema } from "@/lib/order-entry/validation";
...
// PATCH /api/order-entry/users/:id — change role, active flag, name, email, or
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const patch: {
    email?: string;
    name?: string | null;
    role?: "ADMIN" | "SALES" | "OPS" | "VIEWER" | "CRM";
    isActive?: boolean;
    passwordHash?: string;
  } = {};
...
// DELETE /api/order-entry/users/:id — permanently remove an account
// (ADMIN only).
