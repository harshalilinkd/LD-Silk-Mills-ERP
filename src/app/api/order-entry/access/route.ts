...
import { rolePermissions } from "@/db/order-entry/schema";

// GET /api/order-entry/access — the editable role × capability grant matrix
// (ADMIN only). ADMIN is never included (always full). Any (role, capability)
// with no stored row falls back to the code default so the matrix is always
// complete.
...
// PUT /api/order-entry/access — toggle one (role, capability) grant (ADMIN
// only). ADMIN is not editable. Change takes effect on the affected users'
// next request.
