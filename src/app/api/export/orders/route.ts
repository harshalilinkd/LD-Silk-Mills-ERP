...
import { computeLineStatus } from "@/lib/order-entry/workflow";
import {
  customerOrders,
  lineStageProgress,
  orderLineItems,
...
// Ported verbatim from Order Entry's app/api/export/orders/route.ts — only the
// import paths changed. The contract is fixed by two external systems that
// already consume it in production, so nothing about the shape, the auth or
// the ordering may drift.
//
// This route is intentionally OUTSIDE the user-session middleware (see the
// matcher in src/middleware.ts, which excludes `api/export`). Consumers
// authenticate with a static API key in the `x-api-key` header instead. Two
// consume it: the Embroidery System (stock/demand — referred to internally as
// "Knot") and SCOT (the sales-coordinator dashboard). SCOT resolves our
// free-text party names to its own customer master via an alias table, so
// names MUST be passed through verbatim — never trimmed, case-folded or
// otherwise "cleaned", or they arrive as unknown new variants.
//
// Both apps point at the same ld_order_entry data, so this endpoint and the
// standalone app's serve byte-identical results. The keys must therefore be
// the SAME VALUES in both deployments — consumers change only the hostname
// when they cut over, never their credentials.
...
//
// `pricing` is deliberately per-consumer. SCOT asked for it (revenue drives
// its client categorisation); the Embroidery System never did and does not
// need it. Gating on the key keeps the existing contract intact rather than
...
// GET /api/export/orders — read-only incremental pull.
// Pricing is returned ONLY to consumers whose key grants it (SCOT), never to
// Embroidery. Stable ids (order.id, line.id) become each consumer's
// external_ref; stable ordering by (updated_at, id) keeps incremental sync
// reliable. `updated_since` is INCLUSIVE — consumers dedupe on the stable ids,
// so the boundary record may legitimately repeat.
export async function GET(req: Request) {
  ...
  const out = orders.map((o) => ({
    ...
    // SCOT's "gold": the CRR customer this party resolves to, or null where we
    // could not establish it deterministically. NEVER guessed — a null means
    // "we do not know", not "no such customer".
