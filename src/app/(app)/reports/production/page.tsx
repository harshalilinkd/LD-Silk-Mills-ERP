...
import { CountColumns, FunnelRows, Gauge, RankedBars } from "../dashboard-charts";
...
export const metadata: Metadata = { title: "Production dashboard — LD Silk Mills ERP" };
      ...
      {d.total === 0 ? (
        ...
        <>
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_360px]">
            <Card title="Where the work has got to" sub={`${count(d.total)} live lines`}>
              <FunnelRows
                rows={d.stages.map((s) => ({
                  no: s.no,
                  label: s.label,
              ...
              <p className="mt-3 text-[11.5px] leading-snug text-text-3">
                How many lines have finished each stage. The red figure is how many fell away
                since the stage above — the biggest one is the bottleneck.
              </p>
            </Card>
          ...
          <div className="grid gap-3 xl:grid-cols-3">
            ...
            >
              <RankedBars data={d.waiting} money={false} colour="var(--chart-3)" />
            </Card>
            <Card
            ...
            >
              <CountColumns data={d.ageing} height={240} />
            </Card>
            <Card
            ...
            >
              <div className="-mx-1 overflow-x-auto px-1">
                <table className="w-full min-w-[720px] border-collapse">
                  ...
                  <tbody>
                    {d.pending.map((r) => (
                      <tr key={`${r.order_no}|${r.quality}|${r.design}|${r.days_open}|${r.value}`}>
                        <td className={`${td} num whitespace-nowrap`}>{r.order_no}</td>
                        <td className={`${td} max-w-[180px] truncate`} title={r.party}>
                          {r.party}
                        </td>
                        <td className={td}>{r.quality}</td>
                        <td className={td}>{r.design}</td>
                        <td className={`${td} num text-right`}>{qty(r.metres)}</td>
                        <td className={`${td} num text-right`}>{inr(r.value)}</td>
                        <td className={td}>{r.waiting_on}</td>
                        <td className={`${td} num text-right font-semibold text-text-1`}>
                          {count(r.days_open)}
                        </td>
                      </tr>
                    ))}
                ...
                <tbody>
                  {d.stages.map((s) => (
                    <tr key={s.label}>
                      ...
                      <td className={`${td} num text-right`}>{count(s.done)}</td>
                      <td className={`${td} num text-right`}>{pct(s.pct, 0)}</td>
                      <td
          ...
          <div className="grid gap-3 xl:grid-cols-2">
            <Card title="What the numbers say" sub={`${count(d.total)} lines · ${period}`}>
