    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <Figure
          ...
          sub={
            k.late > 0
              ? `Over the ${k.late} finished late`
              : "Nothing finished late"
          }
          delta={
            prev.avgDelay > 0 || k.avgDelay > 0
              ? k.avgDelay - prev.avgDelay
              : undefined
          }
          suffix="d"
          lowerIsBetter
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.5fr]">
        <div className="flex flex-col gap-4">
          ...
          <section className="rounded-card border border-border bg-surface">
            ...
            <p className="px-4 pb-3 text-[11.5px] leading-snug text-text-3">
              The bar is how much work falls on that day; the figure on the
              right is how much of it was on time. Sunday is always empty —
              nothing is ever scheduled on one.
            </p>
          </section>
        </div>
        {/* ── the heatmap ───────────────────────────────────────────── */}
        <section className="rounded-card border border-border bg-surface">
          ...
          <div className="grid grid-cols-1 gap-x-4 gap-y-1 border-t border-border px-4 py-3 text-[11px] text-text-3 sm:grid-cols-2 lg:grid-cols-3">
            <Key
              c="bg-status-green"
              label="On time — everything done by its day"
            />
            <Key
              c="bg-status-amber"
              label="Done late — all done, some after the day"
            />
            <Key
              c="bg-status-red"
              label="Overdue — the day passed, still not done"
            />
            <Key
              c="bg-status-blue"
              label="In progress — due today, not late yet"
            />
            <Key
              c="bg-surface-3"
              label="Scheduled — a future date, not due yet"
            />
                    ...
                    <div className="mt-1 flex h-1.5 overflow-hidden rounded-pill bg-surface-3">
                      {f.onTime > 0 && (
                        <div
                          ...
                          style={{ width: `${pct(f.onTime)}%` }}
                        />
                      )}
                      {f.late > 0 && (
                        <div
                          className="bg-status-red"
                          style={{ width: `${pct(f.late)}%` }}
                        />
                      )}
                    </div>
                  </div>
  ...
  return (
    <div className="rounded-card border border-border bg-surface p-3.5">
      <div className="flex items-start justify-between gap-2">
        ...
        {delta === 0 && (
          <span
            className="inline-flex items-center text-[11px] text-text-3"
            title="Unchanged from the period before"
          >
...
function Heatmap({
  ...
  // Today's cell has to be reachable without already knowing to scroll for
  // it — same reasoning as the Dashboard's month rail. On a range wide enough
  // to scroll, today defaults off in whichever direction the range runs, and
  // that is the one cell somebody opening this screen is most likely here for.
  const scroller = React.useRef<HTMLDivElement>(null);
  const todayCell = React.useRef<HTMLButtonElement>(null);
  React.useEffect(() => {
    const box = scroller.current;
    const btn = todayCell.current;
  ...
  // ── THE CELLS ARE WIDE, NOT SQUARE — ABOVE A FLOOR ──────────────────────
  //
  ...
  // ON A DESKTOP CARD. Below ~460px wide (most phones, in a card with any
  // padding either side) that same math gives ~45px cells still carrying
  // three lines of text (day number, `done/total`, `+Nd` delay), which is
  // what "not looking good on mobile" actually was: not a missing mobile
  // layout, but the fluid width having no floor. `min-w-[460px]` on the grid
  // itself, inside an `overflow-x-auto` wrapper, keeps every cell at least
  // ~58px — the width those three lines need — and lets a narrower screen
  // scroll sideways for it rather than squeeze the pattern unreadable. A
  // heatmap's whole point is comparing cells at a glance; collapsing it into
  // a list on mobile like every other table in this app would lose exactly
  // that, so this scrolls instead of stacking.
  return (
              ...
              {w.map((c, j) =>
                c === null ? (
                  ...
                  >
                    ...
                    {/* The one number that says HOW late, not just that it
                        was. A day showing 5/5 in amber is finished; +3d is
...
function ScoreLegend({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="num w-8 shrink-0 text-right text-[11.5px] text-text-3">
        {pct}%
      </span>
    </div>
  );
  ...
  return [
    {
      ...
      to: today,
      hint: "The last 30 days up to today",
    },
    {
      label: "Last 60 days",
      from: addDays(today, -59),
      to: today,
      hint: "The last 60 days up to today",
    },
    {
      label: "Last 90 days",
      from: addDays(today, -89),
      to: today,
      hint: "The last 90 days up to today",
    },
    {
      label: "This month",
